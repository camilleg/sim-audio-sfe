...
def query_page(title):
    '''
    Return a JSON string fetched from sw.wikipedia.org.
    title is the last part of the URL.
    '''
    if '%' not in title:
        title = urllib.parse.quote_plus(title)

    query = (u'http://sw.wikipedia.org/w/api.php?format=json&action=query&titles={}'
        '&prop=revisions&rvprop=content&redirects').format(title)
    res = urllib.request.urlopen(query)
    return res.read().decode('utf-8')

if __name__ == '__main__':
    t = sys.argv[1] # e.g., Fire_drill for http://sw.wikipedia.org/wiki/Fire_drill
